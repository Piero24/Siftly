...
const timeRangeDeserializer = (raw: string): ('today' | 'total' | '7d' | '30d' | '1y') | null =>
  raw === 'today' || raw === '7d' || raw === '30d' || raw === '1y' || raw === 'total' ? raw : null;
...
export const SettingsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [theme, setTheme] = useLocalStorageState<ThemeMode>(KEYS.theme, 'system', {
    deserialize: themeDeserializer,
  });
  const [defaultTimeRange, setDefaultTimeRange] = useLocalStorageState(
    KEYS.timeRange,
    'total' as const,
    { deserialize: timeRangeDeserializer }
  );
  const [defaultOverviewScope, setDefaultOverviewScope] = useLocalStorageState<OverviewScope>(
    KEYS.overviewScope,
    'total',
    { deserialize: overviewScopeDeserializer }
  );
  const [notifications, setNotifications] = useLocalStorageState(
    KEYS.notifications,
    { email: false },
    jsonSerializer<{ email: boolean }>()
  );
  const [privacy, setPrivacy] = useLocalStorageState(
    KEYS.privacy,
    { telemetry: true, dataRetention: 0 },
    jsonSerializer<{ telemetry: boolean; dataRetention: number }>()
  );
  ...
  useEffect(() => {
    ...
    const update = (e?: MediaQueryListEvent) =>
      setSystemTheme(e ? (e.matches ? 'dark' : 'light') : mq.matches ? 'dark' : 'light');
