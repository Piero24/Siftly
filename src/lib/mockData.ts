...
function buildRounds(index: number, count: number) {
  return Array.from({ length: count }, (_, roundIndex) => {
    ...
    return {
      ...
      meetingLink:
        roundIndex % 2 === 0
          ? `https://meet.example.com/m${index + 1}r${roundIndex + 1}`
          : undefined,
...
function buildApplication(index: number): JobApplication {
  ...
  const application: JobApplication = {
    ...
    description:
      index % 4 === 0
        ? `Role focused on ${POSITIONS[index % POSITIONS.length].toLowerCase()} in ${companyInfo.sector}.`
        : undefined,
  };
