...
export function getOverviewStatusCounts(
  apps: JobApplication[],
  scope: 'total' | 'current'
): StatusCounts {
  const direct = getStatusCounts(apps);
  if (scope === 'current') return direct;

  return {
    total: direct.total,
    pending: direct.pending,
    // In Total mode, Applied is cumulative and includes every application in timeframe.
    applied: direct.total,
    interviewing:
      direct.interviewing + direct.offer + direct.rejected + direct.declined + direct.accepted,
    offer: direct.offer + direct.declined + direct.accepted,
    declined: direct.declined,
    accepted: direct.accepted,
    rejected: direct.rejected,
    'no-response': direct['no-response'],
  };
}
...
export function getTopCompaniesByApplications(apps: JobApplication[], n = 8): CompanyStat[] {
  const map = new Map<
    string,
    { count: number; logo?: string; website?: string; linkedin?: string }
  >();
  for (const app of apps) {
...
export function getTopCompaniesByRejections(apps: JobApplication[], n = 8): CompanyStat[] {
...
export function getTopCitiesByApplications(apps: JobApplication[], n = 8): CityStat[] {
  const map = new Map<string, number>();
...
export function getApplicationTimeline(apps: JobApplication[], weeks = 12): TimelineBucket[] {
  const now = new Date();
  ...
  for (const app of apps) {
    ...
    const appMonday = new Date(
      appDate.getFullYear(),
      appDate.getMonth(),
      appDate.getDate() + appMondayOffset
    );
