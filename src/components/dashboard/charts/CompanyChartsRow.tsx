...
export const CompanyChartsRow: React.FC<CompanyChartsRowProps> = ({
  topApps, topRejections, topCities, isDark, isMobile, useSoftIconBackground,
}) => {
  const chartHeight = Math.max(isMobile ? 220 : 280, 1);

  const CustomYAxisTick = ({ x, y, payload }: any) => {
    if (!payload?.value) return null;
    const stat = topApps.find((a) => a.name === payload.value) || topRejections.find((a) => a.name === payload.value);
      ...
      <div className="db-charts-row">
        ...
        <div className="db-chart-card glass-container">
          ...
          {topRejections.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={chartHeight}>
              <BarChart data={topRejections} layout="vertical" margin={{ top: 4, right: 48, bottom: 4, left: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="var(--border-strong)" horizontal={false} />
                <XAxis type="number" tick={{ fontSize: 11, fill: 'var(--text-secondary)' }} tickLine={false} axisLine={false} allowDecimals={false} />
                <YAxis type="category" dataKey="name" tick={<CustomYAxisTick />} tickLine={false} axisLine={false} width={120} />
                <Bar dataKey="count" radius={[0, 6, 6, 0]} maxBarSize={22}>
