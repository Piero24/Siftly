...
function assertNodeVersion() {
  // node:sqlite requires Node 22.5.0+
  const [major, minor] = process.versions.node.split('.').map(Number);
  if (major < 22 || (major === 22 && minor < 5)) {
    throw new Error(
      `Siftly requires Node.js version 22.5.0 or higher. Current version: ${process.versions.node}.`
    );
  }
}
...
export async function createServer(options = {}) {
  ...
  const metadataPath = options.metadataPath || path.join(__dirname, '../metadata.json');
  const metadata = JSON.parse(fs.readFileSync(metadataPath, 'utf8'));
  const { port: SERVER_PORT, apiBase: API_BASE, host: SERVER_HOST, protocol: SERVER_PROTOCOL } = metadata.server;

  // Robust Database Path (always in project root/data unless overridden)
  const ROOT_DIR = path.resolve(__dirname, '..');
  const defaultDataDir = path.join(ROOT_DIR, 'data');
  const requestedDbPath = options.dbPath || process.env.DB_PATH || path.join(defaultDataDir, 'siftly.db');
  const DB_PATH =
    requestedDbPath === ':memory:'
      ? ':memory:'
      : path.isAbsolute(requestedDbPath)
        ? requestedDbPath
        : path.resolve(ROOT_DIR, requestedDbPath);
  const DATA_DIR = DB_PATH === ':memory:' ? defaultDataDir : path.dirname(DB_PATH);

  // Ensure data directory exists and is writable (skip for in-memory DB)
...
export async function startServer(options = {}) {
  const { server, config } = await createServer(options);
  const PORT = options.port || process.env.PORT || config.SERVER_PORT;
  ...
  console.log(`🚀 Siftly backend running on ${config.SERVER_PROTOCOL}://${config.SERVER_HOST}:${PORT}`);
  console.log(`📂 Database: ${config.DB_PATH}`);

  return { server, port: PORT, dbPath: config.DB_PATH };
}
